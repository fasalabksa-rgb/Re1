...
function submitform(e) {
    // 1. إيقاف إعادة تحميل الصفحة الافتراضي بشكل صحيح
    e.preventDefault(); 

    // 2. التحقق من وجود عناصر مخفية مضافة
    const hiddenContainer = document.getElementById('hiddenInputsContainer');
    ...
    // 3. الإشارة إلى عنصر الفورم بشكل صحيح عبر e.target
    const formElement = e.target;
    const formData = new FormData(formElement);
    // إرسال الطلب لملف الـ PHP
    fetch('purchases.php', {
        method: 'POST',
        body: formData
    })
    .then(response => response.text()) // استقبال الرد كنص (Text)
    .then(scriptText => {
        // تنفيذ النص القادم من السيرفر ككود JavaScript حقيقي
        eval(scriptText);
    })
    .catch(error => {
